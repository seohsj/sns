...
//    }
    ...
    void findAllPhotosOfFollowing() {
        ...
        Result<List<PhotoDto>> result = photoService.findAllPhotosOfFollowing(user1.getId());
        assertThat(result.getData().size()).isEqualTo(6);
        assertTrue(result.getData().stream()
                .allMatch(r -> (
                        r.getUsername().equals("kim") || r.getUsername().equals("lee") || r.getUsername().equals("choi")
                )));
        ...
        Result<List<PhotoDto>> result2 = photoService.findAllPhotosOfFollowing(user2.getId());
        assertThat(result2.getData().size()).isEqualTo(6);
        assertTrue(result2.getData().stream()
                .allMatch(r -> (
                        r.getUsername().equals("seo") || r.getUsername().equals("lee")
                )));
